# -*- coding: utf-8 -*-
# =============================================================================
# 📦 Streamlit 식자재 발주 시스템 (중복ID 패치 + 단가/금액 스냅샷 + 스키마 재정비)
# -----------------------------------------------------------------------------
# 시크릿 예시(변경 금지):
# [users.jeondae]
# password = "jd"
...
# 2) 시트/스키마 정의 (재정비)
# =============================================================================
# 상품마스터(추천): 품목코드, 품목명, 분류, 단위, 단가, 활성
SHEET_NAME_MASTER = "상품마스터"

# 발주 시트(확정): 주문일시, 발주번호, 지점ID, 지점명, 납품요청일, 품목코드, 품목명, 단위, 수량, 단가, 금액, 비고, 상태, 처리일시, 처리자
...
def page_store_register_confirm(master_df: pd.DataFrame):
    ...
    with c1: keyword = st.text_input("품목 검색(이름/코드)", key="store_kw")
    with c2:
    ...
    edited = st.data_editor(
        ...
        use_container_width=True, num_rows="fixed", hide_index=True, height=360, key="store_order_editor"
    )
    ...
    if st.button("📦 발주 제출", type="primary", use_container_width=True, key="store_submit_btn"):
        ...
        # 단가 스냅샷 계산
        price_map = master_df[["품목코드","단가"]].drop_duplicates().set_index("품목코드")["단가"].to_dict()
        ...
        for _, r in sel_df.iterrows():
            ...
            unit_price = int(pd.to_numeric(price_map.get(code, 0), errors="coerce"))
...
def page_store_orders_change():
    ...
    st.markdown("---")
    st.markdown("**발주 변경 (출고 전 건만 수량·단가 수정 가능)**")
    editable = dfv[dfv["상태"]!="출고완료"].copy()
    if editable.empty:
        st.info("출고 전 상태의 발주가 없습니다."); return
    show_cols = ["발주번호","품목코드","품목명","단위","수량","단가","비고"]
    edited = st.data_editor(
        editable[show_cols],
        column_config={
    ...
    if st.button("변경 내용 저장", type="primary", key="store_edit_save"):
        base = df.copy()
        key_cols = ["발주번호","품목코드"]
        merged = base.merge(edited[key_cols+["수량","단가","비고"]],
...
def page_admin_delivery_note(master_df: pd.DataFrame):
    ...
    st.dataframe(dfv, use_container_width=True, height=420)
    buf = make_order_sheet_excel(dfv, include_price=True)  # 관리자: 금액 포함
...
if __name__ == "__main__":
    st.title("📦 식자재 발주 시스템")
    st.caption("중복ID 해결 · 단가/금액 스냅샷 · 스키마 재정비 버전")
