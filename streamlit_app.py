...
def make_order_sheet_excel(df_note: pd.DataFrame, title: str, query_range: str) -> BytesIO:
    ...
    fmt = {
        ...
        "group_total_label": wb.add_format({"bold": True, "bg_color": "#E2EFDA", "border": 1, "align": "center"}),
        "group_total_money": wb.add_format({"bold": True, "num_format": "#,##0", "border": 1, "bg_color": "#E2EFDA"}),
        "grand_total_label": wb.add_format({"bold": True, "bg_color": "#DDEBF7", "border": 1, "align": "center"}),
        "grand_total_money": wb.add_format({"bold": True, "num_format": "#,##0", "border": 1, "bg_color": "#DDEBF7"})
    }
    ...
    current_row = 6
    grouped = df_note.groupby("발주번호")

    for order_id, group in grouped:
        store_name = group['지점명'].iloc[0]
        order_date = pd.to_datetime(group['주문일시'].iloc[0]).strftime('%Y-%m-%d')
        
        ws.merge_range(current_row, 0, current_row, 7, f"■ 지점명: {store_name} / 발주날짜: {order_date} / 발주번호: {order_id}", fmt["header"])
        current_row += 1
        ...
        for item_row, item_data in enumerate(group.itertuples(index=False)):
            row_to_write = current_row + item_row
            ws.write(row_to_write, 1, item_data.납품요청일, fmt["date"])
