...
def create_unified_item_statement(orders_df: pd.DataFrame, supplier_info: pd.Series, customer_info: pd.Series) -> BytesIO:
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        fmt_info_data = workbook.add_format({'font_size': 9, 'align': 'left', 'valign': 'vcenter', 'border': 1, 'text_wrap': True}) # 주소를 위해 줄바꿈 활성화
        ...
        # 3. 레이아웃 설정 (최적화된 12열 그리드)
        # 데이터 목록의 너비에 맞춰 전체 그리드를 재설계
        col_widths = [5, 10, 10, 20, 7, 7, 10, 10, 10, 10, 5, 5] # A-L
        ...
        worksheet.merge_range('D11:F11', date_range, fmt_summary_data)
        worksheet.merge_range('G11:I11', '총 합계 금액', fmt_summary_header)
        worksheet.merge_range('J11:L11', grand_total, fmt_summary_money)
        ...
        for trade_date in df_agg['거래일자'].unique():
            worksheet.merge_range(f'A{current_row}:L{current_row}', f"■ 거래일자 : {trade_date.strftime('%Y년 %m월 %d일')}", fmt_date_header)
            current_row += 1
            related_orders = order_ids_by_date.get(trade_date, "")
            worksheet.merge_range(f'A{current_row}:L{current_row}', f"  관련 발주번호: {related_orders}", fmt_order_id_sub)
            current_row += 1

            # 테이블 헤더 (너비에 맞게 병합 재조정)
            headers = ['No', '품목코드', '품목명', '단위', '수량', '단가', '공급가액', '세액', '합계금액']
            worksheet.write(f'A{current_row}', headers[0], fmt_header) # No (너비 5)
            worksheet.merge_range(f'B{current_row}:C{current_row}', headers[1], fmt_header) # 품목코드 (너비 20)
            worksheet.merge_range(f'D{current_row}:E{current_row}', headers[2], fmt_header) # 품목명 (너비 35)
            worksheet.write(f'F{current_row}', headers[3], fmt_header) # 단위 (너비 7)
            worksheet.write(f'G{current_row}', headers[4], fmt_header) # 수량 (너비 8)
            worksheet.write(f'H{current_row}', headers[5], fmt_header) # 단가 (너비 10)
            worksheet.write(f'I{current_row}', headers[6], fmt_header) # 공급가액 (너비 10)
            worksheet.write(f'J{current_row}', headers[7], fmt_header) # 세액 (너비 10)
            worksheet.merge_range(f'K{current_row}:L{current_row}', headers[8], fmt_header) # 합계금액 (너비 10)
            current_row += 1
            ...
            for _, record in date_df.iterrows():
                ...
                worksheet.merge_range(f'D{current_row}:E{current_row}', record['품목명'], fmt_text_l)
                worksheet.write(f'F{current_row}', record['단위'], fmt_text_c)
                worksheet.write(f'G{current_row}', record['수량'], fmt_money)
                worksheet.write(f'H{current_row}', record['단가'], fmt_money)
                worksheet.write(f'I{current_row}', record['공급가액'], fmt_money)
                worksheet.write(f'J{current_row}', record['세액'], fmt_money)
                worksheet.merge_range(f'K{current_row}:L{current_row}', record['합계금액'], fmt_money)
                current_row += 1
                item_counter += 1

            # 일 계
            worksheet.merge_range(f'A{current_row}:H{current_row}', '일 계', fmt_subtotal_label)
            worksheet.write(f'I{current_row}', date_df['공급가액'].sum(), fmt_subtotal_money)
            worksheet.write(f'J{current_row}', date_df['세액'].sum(), fmt_subtotal_money)
            worksheet.merge_range(f'K{current_row}:L{current_row}', date_df['합계금액'].sum(), fmt_subtotal_money)
            current_row += 2

        # 최종 합계
        worksheet.merge_range(f'A{current_row}:H{current_row}', '총 계', fmt_subtotal_label)
        worksheet.write(f'I{current_row}', df_agg['공급가액'].sum(), fmt_subtotal_money)
        worksheet.write(f'J{current_row}', df_agg['세액'].sum(), fmt_subtotal_money)
        worksheet.merge_range(f'K{current_row}:L{current_row}', df_agg['합계금액'].sum(), fmt_subtotal_money)
