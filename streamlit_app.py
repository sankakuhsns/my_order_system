...
def create_unified_item_statement(orders_df: pd.DataFrame, supplier_info: pd.Series, customer_info: pd.Series) -> BytesIO:
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        worksheet.write(f'I{current_row}', df['합계금액'].sum(), fmt_subtotal_money)
                ...
                with c1:
                    if st.form_submit_button("📦 발주 제출 및 결제", type="primary", use_container_width=True, disabled=not payment_method):
                        ...
                        for _, r in cart_with_master.iterrows():
                            # ✨ [수정] 비고(memo) 필드에 빈 문자열("")을 전달하도록 변경
                            rows.append({"주문일시": now_kst_str(), "발주번호": order_id, "지점ID": user["user_id"], "지점명": user["name"], "품목코드": r["품목코드"], "품목명": r["품목명"], "단위": r["단위"], "수량": r["수량"], "단가": r["단가"], "공급가액": r['공급가액'], "세액": r['세액'], "합계금액": r['합계금액_final'], "비고": "", "상태": CONFIG['ORDER_STATUS']['PENDING'], "처리자": "", "처리일시": "", "반려사유":""})
...
def page_store_orders_change(store_info_df: pd.DataFrame, master_df: pd.DataFrame):
    ...
    # --- 발주 목록 렌더링 ---
    with tab1:
        render_store_order_list(pending, "pending_store")
    with tab2:
        render_store_order_list(shipped, "shipped_store")
    with tab3:
        render_store_order_list(modified, "modified_store")
    with tab4:
        render_store_order_list(rejected, "rejected_store")
...
def render_store_order_details_section(df_all_user_orders: pd.DataFrame, store_info_df: pd.DataFrame, master_df: pd.DataFrame):
    ...
    with st.container(border=True):
        ...
        if not target_df.empty:
            ...
            memo = target_df.iloc[0].get('비고', '')

            st.markdown(f"**선택된 발주번호:** `{target_id}` / **총 합계금액(VAT포함):** `{total_amount:,.0f}원`")

            if pd.notna(memo) and memo.strip():
                st.markdown("**비고 (변동사항 등):**")
                st.text_area("비고_상세", value=memo, height=80, disabled=True, label_visibility="collapsed", key=f"memo_display_{target_id}")
...
def audit_inventory_logs(inventory_log_df, orders_df):
    ...
    - '승인취소'된 원본 주문의 '반려사유'를 분석하여, '변동출고'된 새 주문과 원본 주문을
    ...
    # --- [핵심 수정] 새 주문 ID와 원본 주문 ID를 연결하는 매핑 테이블 생성 ---
    # 이제 '비고' 대신, '승인취소'된 주문의 '반려사유'를 분석하여 더 정확한 연결고리를 찾습니다.
    new_to_original_map = {}
    ...
    for order_id, group in approved_orders.groupby('발주번호'):
        if order_id not in shipment_logged_ids:
            ...
            issues.append(f"- **재고 차감 누락:** 주문 `{order_id}`({store_name})는 '{status}' 상태이나, '발주출고' 기록이 없습니다.")
...
def render_order_edit_modal(order_id: str, df_all: pd.DataFrame, master_df: pd.DataFrame):
    ...
    - '변동출고'된 새 주문의 '비고'란에 원본 주문 ID를 명시적으로 기록하고,
      원본 주문의 '반려사유'란에 새 주문 ID를 기록하여 감사 추적을 100% 보장.
    """
    ...
    with st.form(key="edit_order_form"):
        ...
        if c1.form_submit_button("💾 수정사항 저장", type="primary", use_container_width=True):
            with st.spinner("변경사항 유효성 검사 중..."):
                ...
                items_to_increase = [item for item in inventory_changes if item['수량변경'] < 0]
            ...
            # [시나리오 2] 남은 품목이 하나 이상 -> '주문 수정'으로 처리
                ...
                with st.spinner("안전하게 데이터 업데이트 중..."):
                    try:
                        ...
                        amount_part = ""
                        if price_diff > 0:
                            amount_part = f"{price_diff:,.0f}원 부분환불"
                        elif price_diff < 0:
                            amount_part = f"{abs(price_diff):,.0f}원 추가결제"
                        
                        new_order_id = make_order_id(store_id)
                        ...
                        for _, row in items_to_save.iterrows():
                            ...
                            new_order_rows.append({ 
                                "주문일시": now_kst_str(), 
                                "발주번호": new_order_id,
                                "지점ID": store_id, "지점명": store_name, "품목코드": row['품목코드'], "품목명": row['품목명'], "단위": row['단위'], 
                                "수량": int(row['수량']), "단가": int(row['단가']), "공급가액": supply_price, "세액": tax, "합계금액": supply_price + tax, 
                                "비고": change_log_str, 
                                "상태": CONFIG['ORDER_STATUS']['MODIFIED'], 
                        ...
                        if not update_order_status([order_id], CONFIG['ORDER_STATUS']['CANCELED_ADMIN'], user['name'], reason=f"신규 수정본({new_order_id})으로 대체됨"):
                             raise Exception("수정된 주문서는 추가되었으나, 기존 주문서 상태 변경에 실패했습니다.")
                        
                        add_audit_log(user['user_id'], user['name'], "발주 부분 수정", order_id, store_name, reason=f"신규주문:{new_order_id}, {change_log_str}")
    ...
    with st.container(border=True):
        ...
        if len(selected_ids) == 1:
            ...
            if not target_df.empty:
                ...
                if rejection_reason.strip() and order_status in [CONFIG['ORDER_STATUS']['REJECTED'], CONFIG['ORDER_STATUS']['CANCELED_ADMIN']]:
                    st.error(f"**반려/취소 사유:** {rejection_reason}")
