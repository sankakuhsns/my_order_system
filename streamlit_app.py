            ...
            if st.button("📦 발주 제출", type="primary", use_container_width=True):
                if total_price > total_available:
                    ...
                    for _, r in cart.iterrows():
                        supply_price = r['합계금액']
                        tax = math.ceil(supply_price * 0.1) # 과세/면세 구분 로직 추가 가능
...
def page_store_orders_change(store_info_df: pd.DataFrame, master_df: pd.DataFrame):
    ...
    pending = orders[orders["상태"].isin(["접수", "요청"])].copy()
    shipped = orders[orders["상태"].isin(["승인", "출고완료"])].copy()
    rejected = orders[orders["상태"] == "반려"].copy()

    tab1, tab2, tab3 = st.tabs([f"접수/요청 ({len(pending)}건)", f"승인/출고 ({len(shipped)}건)", f"반려 ({len(rejected)}건)"])
...
def page_store_documents(store_info_df: pd.DataFrame):
    ...
    doc_type = c3.selectbox("서류 종류", ["금전 거래내역서", "세금계산서 (준비중)"], key="store_doc_type")
...
def page_admin_unified_management(df_all: pd.DataFrame, store_info_df: pd.DataFrame, master_df: pd.DataFrame):
    ...
    c1, c2, c3 = st.columns(3)
    status_filter = c1.selectbox("상태", ["전체", "요청", "승인", "반려"], key="admin_status_filter")
    ...
    for order_id, group in order_groups:
        with st.expander(f"{group.iloc[0]['지점명']} - {group.iloc[0]['주문일시']} (발주번호: {order_id}, 상태: {group.iloc[0]['상태']})"):
            ...
            if group.iloc[0]['상태'] == '요청':
                selected = st.checkbox("선택", key=f"select_{order_id}")
                if selected:
                    selected_ids.append(order_id)
    
    if status_filter == '요청' and not selected_ids: st.info("처리할 '요청' 상태의 발주를 선택하세요."); return
    ...
    with c1:
        if st.button("✅ 일괄 승인", use_container_width=True, type="primary"):
            update_order_status(selected_ids, "승인", st.session_state.auth["name"])
            st.session_state.success_message = f"{len(selected_ids)}건의 발주가 '승인' 처리되었습니다."
            st.rerun()
    ...
    with c2:
        ...
        if st.button("❌ 일괄 반려", use_container_width=True):
            if not rejection_reason:
                ...
                st.session_state.success_message = f"{len(selected_ids)}건이 반려 처리되었습니다."
                st.rerun()
...
def page_admin_sales_inquiry(master_df: pd.DataFrame):
    ...
    approved_orders = orders_df[orders_df['상태'].isin(['승인', '출고완료'])].copy()
...
def page_admin_documents(store_info_df: pd.DataFrame):
    ...
    doc_type = c4.selectbox("서류 종류", ["금전 거래내역서", "세금계산서 (준비중)"])
...
def page_admin_items_price(master_df: pd.DataFrame):
    ...
    if st.button("변경된 품목 정보 저장하기", type="primary"):
        save_df_to_sheet(SHEET_NAME_MASTER, edited_df)
