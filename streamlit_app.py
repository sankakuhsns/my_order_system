...
def render_system_audit_tab(store_info_df_raw, master_df_raw, orders_df, balance_df, transactions_df, inventory_log_df, charge_req_df):
    st.markdown("##### 🩺 시스템 점검")
    with st.expander("도움말: 각 점검 항목은 무엇을 의미하나요?", expanded=False):
        st.markdown("""
        각 점검 항목은 우리 시스템의 데이터가 서로 잘 맞물려 정확하게 돌아가고 있는지 확인하는 **'시스템 건강 검진'** 과정입니다.

        * **💰 재무 점검**: `balance_df`(잔액 마스터)의 잔액과 `transactions_df`(거래 내역)의 합계가 일치하는지 확인합니다. 이 항목은 현재 지점의 잔액이 모든 거래 내역과 정합성을 가지고 있는지 검증합니다.
        * **🔗 거래 점검**: `transactions_df`(거래 내역)의 각 항목이 `orders_df`(발주 내역)의 해당 데이터와 올바르게 연결되어 있는지 확인합니다. 거래가 발주와 일치하지 않을 때 문제를 감지합니다.
        * **📦 재고 점검**: `inventory_log_df`(재고 로그)의 입출고 내역이 `orders_df`(발주 내역)에 기록된 출고량과 정확히 일치하는지 확인합니다. 재고 변동 내역이 누락되거나 잘못 기록된 경우를 찾아냅니다.
        * **🏛️ 무결성 점검**: `orders_df`(발주), `transactions_df`(거래), `store_info_df`(지점 정보) 등 주요 데이터프레임 간의 핵심 데이터(예: 지점ID, 상품ID)가 서로 존재하는지 확인합니다. 시스템 전반의 데이터 일관성을 검증합니다.
        """)
