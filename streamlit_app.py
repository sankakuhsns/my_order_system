...
def create_unified_item_statement(orders_df: pd.DataFrame, supplier_info: pd.Series, customer_info: pd.Series) -> BytesIO:
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # 3. 레이아웃 설정 (20열 그리드 시스템)
        # A-I: 공급하는자 (9열), J: 중앙 여백, K-T: 공급받는자 (10열) -> 너비 균형
        col_widths = [2, 8, 8, 4, 4, 4, 4, 4, 4,   # A-I
                      2,                          # J (Gutter)
                      2, 8, 8, 4, 4, 4, 4, 4, 4, 4]  # K-T
        for i, width in enumerate(col_widths):
            worksheet.set_column(i, i, width)

        # 4. 헤더 영역 작성
        worksheet.set_row(0, 40) # 제목 행 높이 증가
        worksheet.merge_range('A1:T1', '품 목 거 래 명 세 서', fmt_title)
        worksheet.merge_range('A2:T2', f"출력일: {datetime.now(KST).strftime('%Y-%m-%d %H:%M:%S')}", fmt_print_date)
        
        worksheet.merge_range('A4:I4', '공급하는자', fmt_subtitle)
        worksheet.merge_range('K4:T4', '공급받는자', fmt_subtitle)

        info_data = [('사업자등록번호', '사업자등록번호'), ('상호', '상호명'), ('대표자', '대표자명'), ('사업장주소', '사업장주소'), ('업태/종목', '업태/종목')]
        
        for i, (label, key) in enumerate(info_data, 5):
            worksheet.set_row(i - 1, 24 if '주소' in label else 18)
            
            # 공급자 정보 (왼쪽)
            worksheet.merge_range(f'A{i}:C{i}', label, fmt_info_label)
            val_sup = f"{supplier_info.get('업태', '')}/{supplier_info.get('종목', '')}" if key == '업태/종목' else supplier_info.get(key, '')
            worksheet.merge_range(f'D{i}:I{i}', val_sup, fmt_info_data)
            
            # 공급받는자 정보 (오른쪽)
            worksheet.merge_range(f'K{i}:M{i}', label, fmt_info_label)
            val_cus = f"{customer_info.get('업태', '')}/{customer_info.get('종목', '')}" if key == '업태/종목' else customer_info.get(key, '')
            worksheet.merge_range(f'N{i}:T{i}', val_cus, fmt_info_data)
        ...
        worksheet.merge_range('A11:D11', '거래 기간', fmt_summary_header)
        worksheet.merge_range('E11:J11', date_range, fmt_summary_data)
        worksheet.merge_range('K11:N11', '총 합계 금액 (VAT 포함)', fmt_summary_header)
        worksheet.merge_range('O11:T11', grand_total, fmt_summary_money)
        ...
        for trade_date in df_agg['거래일자'].unique():
            worksheet.merge_range(f'A{current_row}:T{current_row}', f"■ 거래일자 : {trade_date.strftime('%Y년 %m월 %d일')}", fmt_date_header)
            current_row += 1
            related_orders = order_ids_by_date.get(trade_date, "")
            worksheet.merge_range(f'A{current_row}:T{current_row}', f"  관련 발주번호: {related_orders}", fmt_order_id_sub)
            current_row += 1

            # 테이블 헤더
            headers = ['No', '품목코드', '품목명', '단위', '수량', '단가', '공급가액', '합계금액']
            worksheet.merge_range(f'A{current_row}:B{current_row}', headers[0], fmt_header)
            worksheet.merge_range(f'C{current_row}:E{current_row}', headers[1], fmt_header)
            worksheet.merge_range(f'F{current_row}:J{current_row}', headers[2], fmt_header)
            worksheet.merge_range(f'K{current_row}:L{current_row}', headers[3], fmt_header)
            worksheet.merge_range(f'M{current_row}:N{current_row}', headers[4], fmt_header)
            worksheet.merge_range(f'O{current_row}:P{current_row}', headers[5], fmt_header)
            worksheet.merge_range(f'Q{current_row}:R{current_row}', headers[6], fmt_header)
            worksheet.merge_range(f'S{current_row}:T{current_row}', headers[7], fmt_header)
            current_row += 1

            date_df = df_agg[df_agg['거래일자'] == trade_date]
            item_counter = 1
            for _, record in date_df.iterrows():
                worksheet.merge_range(f'A{current_row}:B{current_row}', item_counter, fmt_text_c)
                worksheet.merge_range(f'C{current_row}:E{current_row}', record['품목코드'], fmt_text_c)
                worksheet.merge_range(f'F{current_row}:J{current_row}', record['품목명'], fmt_text_l)
                worksheet.merge_range(f'K{current_row}:L{current_row}', record['단위'], fmt_text_c)
                worksheet.merge_range(f'M{current_row}:N{current_row}', record['수량'], fmt_money)
                worksheet.merge_range(f'O{current_row}:P{current_row}', record['단가'], fmt_money)
                worksheet.merge_range(f'Q{current_row}:R{current_row}', record['공급가액'], fmt_money)
                worksheet.merge_range(f'S{current_row}:T{current_row}', record['합계금액'], fmt_money)
                current_row += 1
                item_counter += 1

            # 일 계
            worksheet.merge_range(f'A{current_row}:P{current_row}', '일 계', fmt_subtotal_label)
            worksheet.merge_range(f'Q{current_row}:R{current_row}', date_df['공급가액'].sum(), fmt_subtotal_money)
            worksheet.merge_range(f'S{current_row}:T{current_row}', date_df['합계금액'].sum(), fmt_subtotal_money)
            current_row += 2

        # 최종 합계
        worksheet.merge_range(f'A{current_row}:P{current_row}', '총 계', fmt_subtotal_label)
        worksheet.merge_range(f'Q{current_row}:R{current_row}', df_agg['공급가액'].sum(), fmt_subtotal_money)
        worksheet.merge_range(f'S{current_row}:T{current_row}', df_agg['합계금액'].sum(), fmt_subtotal_money)
